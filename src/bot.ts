...
// Extra flags to bypass YouTube datacenter IP blocks
const YT_FLAGS = [
  "--extractor-args", "youtube:player_client=android,web",
  "--add-header", "User-Agent:Mozilla/5.0 (Linux; Android 11; Pixel 5) AppleWebKit/537.36 Chrome/90.0.4430.91 Mobile Safari/537.36",
...
async function playInCall(chatId: number, videoId: string, title: string, uploader: string, api: Bot["api"]) {
  ...
  try {
    ...
    await api.editMessageText(chatId, statusMsg.message_id,
      `❌ فشل التشغيل: ${(err as Error).message}`).catch(() => {});
    if (filePath && existsSync(filePath)) await unlink(filePath).catch(() => {});
  }
}
...
bot.on("message:text", async (ctx) => {
  ...
  if (text.startsWith("يوت ") || text === "يوت") {
    ...
    if (!query) return ctx.reply("⚠️ اكتب اسم الأغنية بعد *يوت*\nمثال: `يوت محمد عبده`", { parse_mode: "Markdown" });
    const searchMsg = await ctx.reply(`🔍 أبحث عن: ${query}`);
    try {
      const results = await searchYouTube(query, 1);
      if (!results.length) { await ctx.api.editMessageText(chatId, searchMsg.message_id, "❌ ما لقيت نتائج."); return; }
      ...
      await ctx.api.editMessageText(chatId, searchMsg.message_id, `❌ صار خطأ:\n\`${msg.slice(0, 300)}\``, { parse_mode: "Markdown" }).catch(() => {});
    }
    return;
  }
  ...
  if (text.startsWith("بحث ") || text === "بحث") {
    const query = text.slice(4).trim();
    if (!query) return ctx.reply("⚠️ اكتب اسم الأغنية بعد *بحث*\nمثال: `بحث ماجد المهندس`", { parse_mode: "Markdown" });
    const searchMsg = await ctx.reply(`🔍 أبحث عن: ${query}`);
    try {
      ...
      for (const v of results) {
        keyboard.text(`${v.title.slice(0, 33)} [${v.duration}]`, `dl:${v.id}:${v.uploader.slice(0, 18)}:${v.title.slice(0, 28)}`).row();
      }
      await ctx.reply(`🎵 *نتائج:* ${query}\n\nاختر أغنية:`, { parse_mode: "Markdown", reply_markup: keyboard });
    } catch (err) {
  ...
  if (text.startsWith("شغل ") || text === "شغل") {
    const query = text.slice(4).trim();
    if (!query) return ctx.reply("⚠️ اكتب اسم الأغنية بعد *شغل*\nمثال: `شغل محمد عبده`", { parse_mode: "Markdown" });
    if (!voiceManager.isReady()) return ctx.reply("❌ خدمة المكالمات غير متاحة. تأكد من /qr أولاً.");
    const searchMsg = await ctx.reply(`🔍 أبحث عن: ${query}`);
    try {
      const results = await searchYouTube(query, 1);
      if (!results.length) { await ctx.api.editMessageText(chatId, searchMsg.message_id, "❌ ما لقيت نتائج."); return; }
  ...
  if (text === "وقف" || text === "⏹ وقف") {
    ...
    if (result.ok) { nowPlaying.delete(chatId); await ctx.reply("⏹ تم إيقاف التشغيل."); }
...
bot.callbackQuery(/^play_call:([^:]+):([^:]+):(.+)$/, async (ctx) => {
  ...
  const chatId = ctx.chat?.id;
  if (!chatId || !videoId || !uploader || !title) return;
  if (!voiceManager.isReady()) { await ctx.reply("❌ خدمة المكالمات غير متاحة. تأكد من /qr أولاً."); return; }
...
export function startBot() {
  ...
  voiceManager.once("ready", async () => {
    ...
    if (check.ok) logger.info({ name: check.name }, "User session active");
    else logger.warn("No active user session — use /qr to log in");
  });
    ...
    if (msg.event === "qr_logged_in") {
      ...
      for (const [userId, chatId] of pendingQR.entries()) {
        try {
          ...
          if (sessionString) text += `\n\n💾 *احفظ Session String:*\n\`${sessionString}\`\n\nضعه في \`TELEGRAM_SESSION_STRING\``;
          await bot.api.sendMessage(chatId, text, { parse_mode: "Markdown" });
        } catch (e) { logger.error({ e, userId }, "Failed to notify QR login"); }
        pendingQR.delete(userId);
      }
    } else if (msg.event === "qr_timeout") {
      for (const [userId, chatId] of pendingQR.entries()) {
        try { await bot.api.sendMessage(chatId, "⏰ انتهت صلاحية رمز QR. استخدم /qr مرة أخرى."); }
        catch (e) { logger.error({ e, userId }, "Failed to notify timeout"); }
        pendingQR.delete(userId);
      }
    } else if (msg.event === "qr_error") {
      for (const [userId, chatId] of pendingQR.entries()) {
        try { await bot.api.sendMessage(chatId, `❌ خطأ في تسجيل الدخول: ${msg.error}`); }
        catch (e) { logger.error({ e }, "Failed to notify QR error"); }
        pendingQR.delete(userId);
      }
    }
  });
  bot.start().catch((err) => logger.error({ err }, "Bot launch error"));
